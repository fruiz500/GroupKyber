...
function encrypt(fileInBin, fileOutName){
	...
	var padding = crypto.getRandomValues(new Uint8Array(200));		//for compatibility with KyberLock; no hidden message; 200 bytes
	...
	setTimeout(function(){										//delay to allow blinker to start
		...
		if(folderMode.checked && !isKeyFile){						//folder mode: symmetric encryption using folderKey as key
			...
			fileOutBin = concatUi8([headTag,recipients,nonce,padding]);	//global output starts with header v1, No. of recipients, 24-byte nonce, 200 bytes of padding			
...
//				padding = fileInBin.slice(headTag.length+25,headTag.length+225),		//not really used
...
//				padding = fileInBin.slice(headTag.length+25,headTag.length+225),	//200 bytes, not used
			...
			if(locksSelected.length != 0){						//senders selected in directory, try those first
				...
				for(var name in GroupKeys){						//include also legacy users
					var thisLockStr = GroupKeys[name][0];
